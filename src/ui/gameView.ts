...
export function gameView(root:HTMLElement,rawRoom:Room,code:string,player:Player,connected:boolean,opponentOnline:boolean,busy:boolean,move:(i:number)=>void,leave:()=>void,presentation:GamePresentation={}){
  ...
  if(root.dataset.room!==code||!root.querySelector(".board")){
    ...
    root.innerHTML='<h1>ReverSix!</h1><div class="room"><span></span><button class="copy">COPY</button></div><div class="game-status-area"><h2 class="turn-status" role="status"></h2><p class="result-detail status-empty"></p><p class="check status-empty"></p></div><div class="game-layout clock-board-layout"><aside class="player-clock clock-left"><span class="clock-color"></span><strong class="clock-time"></strong></aside><div class="board-wrap"><div class="board-slot"></div></div><aside class="player-clock clock-right"><span class="clock-color"></span><strong class="clock-time"></strong></aside></div><p class="you"></p><p class="notice status-empty" role="status"></p><div class="game-controls"><button class="rematch" hidden></button><button class="change-options" hidden></button><button class="commit" disabled></button><button class="undo" disabled>UNDO</button><button class="back">BACK TO LOBBY</button></div><p class="game-error" role="alert" hidden></p><div class="result-overlay" hidden aria-live="assertive"><strong></strong></div><dialog class="timeout-dialog" aria-modal="true"></dialog>';
  ...
  const myTurn=!finished&&room.status==="playing"&&s.currentPlayer===player&&!presentation.reviewing;
  const noMoves=myTurn&&Boolean(presentation.passing);
  const noticeText=!connected?t("game.connection"):room.players.white&&!opponentOnline?t("game.disconnected"):finished?"":noMoves?t("game.noMoves"):s.events.join(" · ");
  ...
  const commit=root.querySelector<HTMLButtonElement>(".commit")!;commit.hidden=!myTurn;commit.textContent=t(noMoves?"game.passTurn":"game.commitTurn");commit.classList.toggle("passing",noMoves);commit.disabled=!presentation.canCommit||busy||!connected||!opponentOnline||countingDown||Boolean(timeout.pendingFor);commit.onclick=()=>{if(!commit.disabled)presentation.commit?.()};
