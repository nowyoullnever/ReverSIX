...
export function gameView(root:HTMLElement,rawRoom:Room,code:string,player:Player,connected:boolean,opponentOnline:boolean,busy:boolean,move:(i:number)=>void,leave:()=>void,presentation:GamePresentation={}){
  ...
  if(root.dataset.room!==code||!root.querySelector(".board")){
    ...
    root.innerHTML='<h1>ReverSix!</h1><div class="room"><span></span><button class="copy">COPY</button></div><h2 class="turn-status" role="status"></h2><p class="result-detail" hidden></p><p class="check" hidden></p><div class="game-layout clock-board-layout"><aside class="player-clock clock-left"><span class="clock-color"></span><strong class="clock-time"></strong></aside><div class="board-wrap"><div class="board-slot"></div></div><aside class="player-clock clock-right"><span class="clock-color"></span><strong class="clock-time"></strong></aside></div><p class="you"></p><p class="notice" role="status"></p><div class="game-controls"><button class="rematch" hidden></button><button class="change-options" hidden></button><button class="text-button undo" disabled>UNDO</button><button class="back">BACK TO LOBBY</button></div><p class="game-error" role="alert" hidden></p><div class="countdown-overlay" hidden aria-live="assertive"></div><div class="result-overlay" hidden aria-live="assertive"><strong></strong></div><dialog class="timeout-dialog" aria-modal="true"></dialog>';
    root.querySelector(".board-slot")!.append(boardWithCoordinates(s,false,move,finalPresentation));
  }
  ...
  const check=root.querySelector<HTMLElement>(".check")!;check.hidden=finished||!s.checkBy;check.textContent=s.checkBy?s.checkBy===player?t("game.check"):t("game.defend"):"";
  const detail=root.querySelector<HTMLElement>(".result-detail")!;detail.hidden=!defenseFailed;detail.textContent=defenseFailed?sessionGame.winner===player?t("game.survived"):t("game.remained"):"";detail.classList.toggle("result-enter",Boolean(defenseFailed&&presentation.defeatSequence&&!presentation.reviewing));
  ...
  const previousNotice=notice.dataset.message;setStatusText(notice,noticeText,!connected);notice.dataset.message=noticeText;if(room.players.white&&!opponentOnline&&connected&&previousNotice!==noticeText)replayMotion(notice,"notice-enter");
