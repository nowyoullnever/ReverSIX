...
export function localGameView(root:HTMLElement,game:GameState,busy:boolean,move:(index:number)=>void,leave:()=>void,presentation:LocalGamePresentation){
  ...
  if(root.dataset.mode!==mode||!root.querySelector(".board")){
    ...
    root.innerHTML='<h1>ReverSix!</h1><p class="local-title"></p><h2 class="turn-status" role="status"></h2><p class="computer-status" role="status" hidden></p><p class="check" hidden></p><div class="clock-board-layout"><aside class="player-clock clock-left"><span class="clock-color"></span><strong class="clock-time"></strong></aside><div class="board-wrap"><div class="board-slot"></div></div><aside class="player-clock clock-right"><span class="clock-color"></span><strong class="clock-time"></strong></aside></div><p class="local-count"></p><p class="notice" role="status"></p><div class="game-controls"><button class="rematch" hidden></button><button class="change-options" hidden></button><button class="text-button undo" disabled></button><button class="back"></button></div><p class="game-error" role="alert" hidden></p><div class="countdown-overlay" hidden aria-live="assertive"></div><div class="result-overlay" hidden aria-live="assertive"><strong></strong></div><dialog class="timeout-dialog" aria-modal="true"></dialog>';
