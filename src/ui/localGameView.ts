...
export function localGameView(root:HTMLElement,game:GameState,busy:boolean,move:(index:number)=>void,leave:()=>void,presentation:LocalGamePresentation){
  ...
  const thinking=root.querySelector<HTMLElement>(".computer-status")!;thinking.textContent=!presentation.computerThinking||finished?"":`${t("computer.thinking")}${presentation.computerProgress?` ${presentation.computerProgress.done} / ${presentation.computerProgress.total}`:""}`;thinking.classList.toggle("status-empty",!thinking.textContent);
