...
import { searchPlacement } from "./search";

type Request={type:"load";id:number;base:string}|{type:"search";id:number;state:GameState}|{type:"cancel";id:number};
...
self.onmessage=async(event:MessageEvent<Request>)=>{
  ...
  try{
    ...
    const index=await searchPlacement(network,request.state,{sims:32,candidates:16,cancelled:()=>cancelled.has(request.id),onProgress:async(done,total)=>{send({type:"progress",id:request.id,done,total});await new Promise(resolve=>setTimeout(resolve,0))}});
