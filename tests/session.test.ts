...
it("charges a whole turn's thinking time when its player confirms it",()=>{const game=new LocalGameSession(DEFAULT_SETTINGS,0);expect(()=>game.play(34,2999)).toThrow("GAME IS COUNTING DOWN");expect(game.remaining("black",3000)).toBe(300000);game.play(34,5000);expect(game.game.currentPlayer).toBe("black");game.commit(5000);expect(game.remaining("black",8000)).toBe(298000);expect(game.game.currentPlayer).toBe("white");game.play(getMoveOptions(game.game).legal[0],8000);expect(game.game.currentPlayer).toBe("white");expect(game.canCommit).toBe(false);game.play(getMoveOptions(game.game).legal[0],9000);expect(game.canCommit).toBe(true);game.commit(10000);expect(game.remaining("white",10000)).toBe(295000);expect(game.game.currentPlayer).toBe("black")});
it("local ALL takes back the stone in hand, then whole committed turns",()=>{const game=new LocalGameSession(DEFAULT_SETTINGS,0);game.play(34,10000);game.commit(10000);game.play(33,15000);game.undo(20000);expect(game.game.turnPlacements).toEqual([]);expect(game.game.currentPlayer).toBe("white");game.undo(21000);expect(game.game.currentPlayer).toBe("black");expect(game.turnLog).toHaveLength(0);expect(game.remaining("black",21000)).toBe(300000)});
it("local TURN only rewinds the turn still in hand",()=>{const game=new LocalGameSession({...DEFAULT_SETTINGS,undoMode:"turn",checkRingEnabled:true},0);expect(game.canUndo(4000)).toBe(false);game.play(34,4000);expect(game.canUndo(4000)).toBe(true);game.commit(4000);expect(game.canUndo(4500)).toBe(false)});
...
it("disables elapsed time and timeout when the time limit is off",()=>{const game=new LocalGameSession({...DEFAULT_SETTINGS,clockEnabled:false},0);expect(game.clock.running).toBe(false);expect(game.tick(999999)).toBe(false);game.play(34,999999);game.commit(999999);expect(game.clock.blackRemainingMs).toBe(300000);expect(game.turnLog[0].elapsedMs).toBe(0)});
