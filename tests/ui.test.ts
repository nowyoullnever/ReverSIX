...
it.each([true,false])("ends the online countdown and enables BLACK whether clocks are %s",clockEnabled=>{const root=document.createElement("main"),base={status:"playing" as const,createdAt:1,players:{black:"a",white:"b"},settings:{...DEFAULT_SETTINGS,clockEnabled},clock:{blackRemainingMs:300000,whiteRemainingMs:300000,activeSince:4000,running:clockEnabled},game:createGame(),countdownEndsAt:4000};gameView(root,base,"ABC234","black",true,true,false,vi.fn(),vi.fn(),{now:3000});expect(root.querySelector<HTMLElement>(".countdown-overlay")!.hidden).toBe(false);gameView(root,base,"ABC234","black",true,true,false,vi.fn(),vi.fn(),{now:4000});expect(root.querySelector<HTMLElement>(".countdown-overlay")!.hidden).toBe(true);expect(root.querySelectorAll(".cell:not(:disabled)")).toHaveLength(4)});
