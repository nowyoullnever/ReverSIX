...
function loadModel(){const meta=JSON.parse(readFileSync("public/model/latest.json","utf8")) as ModelMeta,buffer=readFileSync("public/model/latest.bin"),flat=new Float32Array(buffer.buffer.slice(buffer.byteOffset,buffer.byteOffset+buffer.byteLength));return new ReversixNet(meta,flat)}

it("matches the upstream iteration-20 inference test vector",()=>{const net=loadModel(),vector=JSON.parse(readFileSync("public/model/latest.testvec.json","utf8")) as {input:number[];policy:number[];value:number},output=net.forward(Float32Array.from(vector.input));expect(net.meta).toMatchObject({board:10,planes:9,ch:64,blocks:6,iter:20,total_floats:454754});expect(output.policy).toHaveLength(100);expect(Math.max(...output.policy.map((value,index)=>Math.abs(value-vector.policy[index])))).toBeLessThan(1e-4);expect(Math.abs(output.value-vector.value)).toBeLessThan(1e-4)});
