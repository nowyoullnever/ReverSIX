...
it("updates theme, sound, and BGM selected states from live getters",()=>{
  let sound=false,bgm=true,theme:Theme="light";const changed=vi.fn();applyTheme(theme);
  const dialog=openSettings({getSound:()=>sound,setSound:value=>{sound=value},getTheme:()=>theme,setTheme:value=>{theme=value;applyTheme(value)},getBgm:()=>bgm,setBgm:value=>{bgm=value},changed});
  expect([...dialog.querySelectorAll(".settings-label")].map(label=>label.textContent)).toEqual(["LANGUAGE","APPEARANCE","SOUND","BGM"]);
